...
{
   public class Verb_BestState : Verb_CastAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (ability.comps[i] is rimbreak.RbAbilityComps.AbilityComp_BestState)
                {
                    targetComp = ability.comps[i];
                    break;
                }
            }
            var comp = targetComp as rimbreak.RbAbilityComps.AbilityComp_BestState;
            if (comp == null)
            {
                return false;
            }
            ...
            // 伤害半径（5x5格）
            int radius = 4; // 5x5范围实际上是以pawn为中心，半径2格
            float damageAmount = 50f;
            Hediff hediff = HediffMaker.MakeHediff(RbHediffDef.RB_BriarCrown, pawn);
