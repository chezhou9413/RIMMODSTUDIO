...
public class Pathing : IDisposable
{
	...
	{
		PathGridDef pathGridDef = default(PathGridDef);
		PathingContext pathingContext = default(PathingContext);
		foreach (KeyValuePair<PathGridDef, PathingContext> context in contexts)
		{
			context.Deconstruct(ref pathGridDef, ref pathingContext);
	...
	public void RecalculatePerceivedPathCostAt(IntVec3 c)
	{
		...
		{
			context.Deconstruct(ref pathGridDef, ref pathingContext);
			pathingContext.pathGrid.RecalculatePerceivedPathCostAt(c, ref haveNotified);
		}
	...
	public void DisableIncrementalDirtying()
	{
		...
		{
			context.Deconstruct(ref pathGridDef, ref pathingContext);
			pathingContext.pathGrid.DisableIncrementalDirtying();
		}
	...
	public void ReEnableIncrementalDirtying()
	{
		...
		{
			context.Deconstruct(ref pathGridDef, ref pathingContext);
			pathingContext.pathGrid.ReEnableIncrementalDirtying();
		}
