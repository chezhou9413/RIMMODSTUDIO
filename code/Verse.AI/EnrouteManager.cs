...
public class EnrouteManager : IExposable
{
	...
	public void LogEnroute()
	{
		...
		List<ThingCountTracker.PawnCount> list = default(List<ThingCountTracker.PawnCount>);
		for (int i = 0; i < enroute.Count; i++)
		{
			...
			foreach (KeyValuePair<ThingDef, List<ThingCountTracker.PawnCount>> readOnlyPair in thingCountTracker.ReadOnlyPairs)
			{
				readOnlyPair.Deconstruct(ref thingDef, ref list);
				ThingDef thingDef2 = thingDef;
				List<ThingCountTracker.PawnCount> list2 = list;
