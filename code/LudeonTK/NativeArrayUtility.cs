...
public static class NativeArrayUtility
{
	...
	{
		//IL_0000: Unknown result type (might be due to invalid IL or missing references)
		UnsafeUtility.MemClear(NativeListUnsafeUtility.GetUnsafePtr<T>(list), (long)list.Length * (long)sizeof(T));
	}
	...
	public static NativeList<T> EmptyList<T>() where T : unmanaged
	{
		...
		return new NativeList<T>(0, AllocatorHandle.op_Implicit(Allocator.Persistent));
	}
	...
	public static NativeBitArray EmptyBitArray()
	{
		...
		return new NativeBitArray(0, AllocatorHandle.op_Implicit(Allocator.Persistent), NativeArrayOptions.ClearMemory);
	}
}
