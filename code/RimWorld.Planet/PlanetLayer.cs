...
{
	public delegate float CalculateAverageTileSize_000152CA_0024PostfixBurstDelegate(in NativeArray<int> tileIDToNeighbors_offsets, in NativeArray<PlanetTile> tileIDToNeighbors_values, in NativeArray<int> tileIDToVerts_offsets, in NativeArray<Vector3> verts);

	internal static class CalculateAverageTileSize_000152CA_0024BurstDirectCall
	{
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(CalculateAverageTileSize_000152CA_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static CalculateAverageTileSize_000152CA_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	public delegate float IntGetTileSize_000152CC_0024PostfixBurstDelegate(in NativeArray<int> tileIDToNeighbors_offsets, in NativeArray<PlanetTile> tileIDToNeighbors_values, in NativeArray<int> tileIDToVerts_offsets, in NativeArray<Vector3> verts, in PlanetTile tile);

	internal static class IntGetTileSize_000152CC_0024BurstDirectCall
	{
		private static IntPtr Pointer;
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(IntGetTileSize_000152CC_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static IntGetTileSize_000152CC_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	public delegate void IntGetTileCenter_000152CF_0024PostfixBurstDelegate(in NativeArray<int> tileIDToVerts_offsets, in NativeArray<Vector3> verts, int tileID, out Vector3 center);

	internal static class IntGetTileCenter_000152CF_0024BurstDirectCall
	{
		private static IntPtr Pointer;
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(IntGetTileCenter_000152CF_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static IntGetTileCenter_000152CF_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	{
		return CalculateAverageTileSize_000152CA_0024BurstDirectCall.Invoke(in tileIDToNeighbors_offsets, in tileIDToNeighbors_values, in tileIDToVerts_offsets, in verts);
	}
	...
	{
		return IntGetTileSize_000152CC_0024BurstDirectCall.Invoke(in tileIDToNeighbors_offsets, in tileIDToNeighbors_values, in tileIDToVerts_offsets, in verts, in tile);
	}
	...
	{
		IntGetTileCenter_000152CF_0024BurstDirectCall.Invoke(in tileIDToVerts_offsets, in verts, tileID, out center);
	}
	...
	{
		return dist / ((float)Math.PI * 2f * Radius) * 360f;
	}
	...
	public void ExposeData()
	{
		...
		if (Scribe.mode == LoadSaveMode.PostLoadInit && ModsConfig.OdysseyActive)
		{
			PlanetLayer planetLayer = Find.WorldGrid.FirstLayerOfDef(PlanetLayerDefOf.Surface);
			PlanetLayer planetLayer2 = (planetLayer.zoomOutToLayer = Find.WorldGrid.FirstLayerOfDef(PlanetLayerDefOf.Orbit));
			if (planetLayer2 != null)
			{
				planetLayer2.zoomInToLayer = planetLayer;
			}
		}
	}
	...
	protected virtual void RawDataToTiles()
	{
		if (tiles.Count != TilesCount)
		{
			...
			{
				Tile tile = (Tile)Activator.CreateInstance(def.tileType, new PlanetTile(i, this));
				tiles.Add(tile);
	...
	private bool IsConnectedToRecursive(PlanetLayer other)
	{
		...
		PlanetLayerConnection planetLayerConnection = default(PlanetLayerConnection);
		foreach (KeyValuePair<PlanetLayer, PlanetLayerConnection> connection in connections)
		{
			connection.Deconstruct(ref planetLayer, ref planetLayerConnection);
	...
	public static bool TryGetPath(PlanetLayer start, PlanetLayer goal, List<PlanetLayerConnection> path, out float cost, int max = int.MaxValue)
	{
		...
		while (openSet.Count != 0)
		{
			...
			{
				connection.Deconstruct(ref planetLayer2, ref planetLayerConnection);
				PlanetLayer planetLayer3 = planetLayer2;
				PlanetLayerConnection planetLayerConnection2 = planetLayerConnection;
