...
public class WorldGrid : IExposable, IDisposable
{
	...
	private void CreateRequiredLayers()
	{
		...
		List<LayerConnection> list = default(List<LayerConnection>);
		foreach (KeyValuePair<PlanetLayer, List<LayerConnection>> item in dictionary)
		{
			item.Deconstruct(ref planetLayer2, ref list);
			PlanetLayer planetLayer3 = planetLayer2;
			foreach (LayerConnection item2 in list)
			{
				if (TryGetLayerByTag(item2.tag, out var layer2) && !planetLayer3.HasConnectionFromTo(layer2))
				{
					planetLayer3.AddConnection(layer2, item2.fuelCost);
					if (item2.zoomMode == LayerConnection.ZoomMode.ZoomIn)
					{
						planetLayer3.zoomInToLayer = layer2;
					}
					else if (item2.zoomMode == LayerConnection.ZoomMode.ZoomOut)
					{
	...
	public void ExposeData()
	{
		...
		if (Scribe.mode == LoadSaveMode.LoadingVars)
		{
			...
			else
			{
				...
				{
					planetLayer4.Deconstruct(ref num, ref planetLayer);
					int item = num;
					PlanetLayer planetLayer2 = planetLayer;
					if (planetLayer2.Def == null)
					{
						tmpLayerIds.Add(item);
						continue;
					}
					if (planetLayer2.IsRootSurface)
					{
						surface = (SurfaceLayer)planetLayer2;
					}
					if (ModsConfig.OdysseyActive && planetLayer2.Def == PlanetLayerDefOf.Orbit)
					{
						orbit = planetLayer2;
					}
					if (ModsConfig.OdysseyActive && planetLayer2.Def == PlanetLayerDefOf.Orbit && planetLayer2.ScenarioTag == null)
					{
						planetLayer2.ScenarioTag = "Orbit";
					}
				}
				if (surface == null)
				{
					foreach (KeyValuePair<int, PlanetLayer> planetLayer5 in planetLayers)
					{
						planetLayer5.Deconstruct(ref num, ref planetLayer);
						PlanetLayer planetLayer3 = planetLayer;
						if (planetLayer3.Def == PlanetLayerDefOf.Surface)
						{
							surface = (SurfaceLayer)planetLayer3;
							surface.MarkRootSurface();
							break;
						}
					}
