...
public class Gravship : WorldObject, IIncidentTarget, ILoadReferenceable
{
	...
	public float PlayerWealthForStoryteller
	{
		get
		{
			...
			PositionData positionData = default(PositionData);
			foreach (KeyValuePair<Pawn, PositionData> pawn3 in pawns)
			{
				pawn3.Deconstruct(ref pawn, ref positionData);
				Pawn pawn2 = pawn;
				num += WealthWatcher.GetEquipmentApparelAndInventoryWealth(pawn2);
				if (pawn2.Faction == Faction.OfPlayer)
				{
					float num2 = pawn2.MarketValue;
					if (pawn2.IsSlave)
					{
						num2 *= 0.75f;
					}
	...
	private void CopyAreas(Map oldMap, IntVec3 origin, HashSet<IntVec3> engineFloors)
	{
		foreach (Area area in oldMap.areaManager.AllAreas)
		{
			if (area is Area_Home)
			{
				...
				areas.homeArea.assignedPawns.AddRange(oldMap.mapPawns.AllPawnsSpawned.Where((Pawn x) => x.playerSettings?.AreaRestrictionInPawnCurrentMap == area));
				continue;
			}
			if (area is Area_Allowed area2)
			{
				...
				moveableArea_Allowed.assignedPawns.AddRange(oldMap.mapPawns.AllPawnsSpawned.Where((Pawn x) => x.playerSettings?.AreaRestrictionInPawnCurrentMap == area));
	...
	private void AddThing(Thing thing, IntVec3 offset)
	{
		...
		if (thing is Pawn pawn)
		{
			...
			pawn.playerSettings?.Notify_MapRemoved(thing.Map);
		}
		else
		{
		...
		if (thing.TryGetComp(out CompGravshipThruster comp2) && comp2.CanBeActive)
		{
			thrusters.Add(thing, new PositionData(offset, thing.Rotation));
			launchDirection += thing.Rotation.AsIntVec3 * comp2.Props.directionInfluence;
		}
	...
	private void CheckAffectedGoodwill()
	{
		...
		foreach (KeyValuePair<Pawn, PositionData> pawn3 in pawns)
		{
			...
			if (GravshipUtility.ConsideredKidnapped(engine, pawn2))
			{
				...
				num = dictionary[faction]++;
			}
		}
		foreach (KeyValuePair<Faction, int> item in tmpFactionMembersKidnappedCount)
		{
			item.Deconstruct(ref faction, ref num);
			faction.TryAffectGoodwillWith(goodwillChange: -10 * num, other: Faction.OfPlayer, canSendMessage: true, canSendHostilityLetter: true, reason: HistoryEventDefOf.PawnKidnappedOnGravShip);
		}
	}
	...
	private Dictionary<T, PositionData.Data> GetPlacementValues<T>(Dictionary<T, PositionData> dictionary, ref Dictionary<T, PositionData.Data> working, ref Rot4 prevRotation) where T : Thing
	{
		...
		{
			item.Deconstruct(ref val, ref positionData);
			T val2 = val;
			PositionData positionData2 = positionData;
			IntVec3 center = PrefabUtility.GetAdjustedLocalPosition(positionData2.position, rotation);
			IntVec2 size = val2.def.size;
			bool flag = true;
			if (!val2.def.rotatable && size.x == size.z)
			{
				GenAdj.AdjustForRotation(ref center, ref size, rotation);
				flag = false;
			}
			else if (!val2.def.rotatable && val2.def.category != ThingCategory.Building)
			{
				flag = false;
			}
			Rot4 rot = ((!flag) ? Rot4.North : rotation.Rotated(positionData2.relativeRotation));
			working[val2] = new PositionData.Data
			{
				local = center,
				rotation = rot,
				drafted = positionData2.drafted
			};
		}
	...
	private Dictionary<IntVec3, T> GetRotatedValues<T>(Dictionary<IntVec3, T> dictionary, ref Dictionary<IntVec3, T> working, ref Rot4 prevRotation)
	{
		...
		{
			item.Deconstruct(ref intVec, ref val);
			IntVec3 local = intVec;
			T value = val;
	...
	private IEnumerable<(IntVec3, T)> GetRotatedValues<T>(Dictionary<IntVec3, T> dictionary)
	{
		...
		{
			item2.Deconstruct(ref intVec, ref val);
			IntVec3 local = intVec;
			T item = val;
