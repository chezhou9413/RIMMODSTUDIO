...
public class FastTileFinder : IDisposable
{
	...
	{
		public delegate float SphericalDistance_00014E0B_0024PostfixBurstDelegate(in float3 a, in float3 b);

		internal static class SphericalDistance_00014E0B_0024BurstDirectCall
		{
			...
			private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(SphericalDistance_00014E0B_0024PostfixBurstDelegate).TypeHandle);
				}
				P_0 = Pointer;
			}
			...
			static SphericalDistance_00014E0B_0024BurstDirectCall()
			{
				Constructor();
			}
		...
		{
			return SphericalDistance_00014E0B_0024BurstDirectCall.Invoke(in a, in b);
		}
	...
	public List<PlanetTile> Query(TileQueryParams query, List<BiomeDef> biomes = null, List<LandmarkDef> landmarks = null, TileQueryParams desperate = default(TileQueryParams))
	{
		...
		{
			ParameterizeJob(query, desperate, nBiomes, nLandmarks).Schedule(cachedTileData.Length, UnityData.GetIdealBatchCount(cachedTileData.Length)).Complete();
		}
		nBiomes.Dispose();
		nLandmarks.Dispose();
	...
	public bool Closest(TileQueryParams query, out PlanetTile tile, List<BiomeDef> biomes = null, List<LandmarkDef> landmarks = null, TileQueryParams desperate = default(TileQueryParams))
	{
		...
		using (ProfilerBlock.Scope("Compute Tile Query"))
		{
			...
			jobData.Schedule(cachedTileData.Length, UnityData.GetIdealBatchCount(cachedTileData.Length)).Complete();
		}
		nBiomes.Dispose();
		nLandmarks.Dispose();
	...
	private ComputeQueryJob ParameterizeJob(TileQueryParams query, TileQueryParams desperate, NativeArray<ushort> nBiomes, NativeArray<ushort> nLandmarks)
	{
		...
		return new ComputeQueryJob
		{
			...
			countNormal = 0,
			countDesperate = 0,
			vOrigin = float3.op_Implicit(origin.Layer.GetTileCenter(origin)),
			region = region
		};
	}
	...
	public static void Initialize_0024ComputeQueryJob_SphericalDistance_00014E0B_0024BurstDirectCall()
	{
		ComputeQueryJob.SphericalDistance_00014E0B_0024BurstDirectCall.Initialize();
	}
}
