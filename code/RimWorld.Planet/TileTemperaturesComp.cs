...
public class TileTemperaturesComp : WorldComponent
{
	...
	{
		PlanetLayerDef planetLayerDef = default(PlanetLayerDef);
		List<int> list = default(List<int>);
		foreach (KeyValuePair<PlanetLayerDef, List<int>> usedSlot in usedSlots)
		{
			usedSlot.Deconstruct(ref planetLayerDef, ref list);
			PlanetLayerDef key = planetLayerDef;
			foreach (int item in list)
			{
				cache[key][item].CheckCache();
			}
		}
		...
		{
			usedSlot2.Deconstruct(ref planetLayerDef, ref list);
			PlanetLayerDef key2 = planetLayerDef;
			List<int> list2 = list;
			if (list2.Any())
			{
				cache[key2][list2[0]] = null;
				list2.RemoveAt(0);
			}
		}
	}
