...
public class Alert_RoyalNoAcceptableFood : Alert
{
	...
	public List<Pawn> Targets
	{
		get
		{
			...
			CachedResult cachedResult = default(CachedResult);
			...
			for (int i = 0; i < maps.Count; i++)
			{
				foreach (Pawn item in maps[i].mapPawns.FreeColonistsSpawned)
				{
					...
					bool flag = false;
					foreach (KeyValuePair<CachedResult, bool> cachedResult3 in cachedResults)
					{
						cachedResult3.Deconstruct(ref cachedResult, ref flag2);
						CachedResult cachedResult2 = cachedResult;
						bool flag3 = flag2;
