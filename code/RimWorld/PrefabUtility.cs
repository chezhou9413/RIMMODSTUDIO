...
public static class PrefabUtility
{
	...
	public static PrefabDef CreatePrefab(CellRect rect, bool copyAllThings = false, bool copyTerrain = false)
	{
		...
		if (copyTerrain)
		{
			...
			(TerrainDef, ColorDef) tuple = default((TerrainDef, ColorDef));
			List<IntVec3> list3 = default(List<IntVec3>);
			foreach (KeyValuePair<(TerrainDef, ColorDef), List<IntVec3>> item5 in dictionary)
			{
				item5.Deconstruct(ref tuple, ref list3);
				(TerrainDef, ColorDef) tuple2 = tuple;
				TerrainDef item = tuple2.Item1;
				ColorDef item2 = tuple2.Item2;
				List<IntVec3> list4 = list3;
