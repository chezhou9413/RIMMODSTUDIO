...
public class LavaEmergence : Thing
{
	...
	protected virtual IEnumerable<IntVec3> GetInitialCells(Map map)
	{
		...
		foreach (IntVec3 item in lavaDeepCells.ToList())
		{
			...
			foreach (IntVec3 intVec in cardinalDirections)
			{
				...
				if (c.InBounds(map) && c.GetTerrain(map) != TerrainDefOf.LavaDeep)
				{
					flag = true;
					break;
				}
	...
	protected void BeginCooling()
	{
		...
		while (!cells.Empty())
		{
			IntVec3 intVec = cells.RandomElementByWeight(delegate(IntVec3 c)
			{
				int num2 = 0;
				IntVec3[] cardinalDirections = GenAdj.CardinalDirections;
				foreach (IntVec3 intVec2 in cardinalDirections)
				{
					...
					if (intVec3.InBounds(base.Map) && (cells.Contains(intVec3) || intVec3.GetTerrain(base.Map) == TerrainDefOf.LavaDeep))
					{
						num2++;
					}
				}
			...
			base.Map.tempTerrain.QueueRemoveTerrain(intVec, Find.TickManager.TicksGame + coolDelay + 400 * num);
			cells.Remove(intVec);
			num++;
		}
		Destroy();
