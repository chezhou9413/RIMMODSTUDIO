...
public static class DropCellFinder
{
	...
	public static IntVec3 FindRaidDropCenterDistant(Map map, bool allowRoofed = false)
	{
		...
		int num = 0;
		float num2 = 65f;
		foreach (IntVec3 item in map.AllCells.Where((IntVec3 cell) => CanPhysicallyDropInto(cell, map, canRoofPunch: true, allowedIndoors: false) && !cell.Fogged(map) && (allowRoofed || !cell.Roofed(map))).InRandomOrder())
		{
			num++;
			if (num > 300)
			{
	...
	public static bool CanPhysicallyDropInto(IntVec3 c, Map map, bool canRoofPunch, bool allowedIndoors = true)
	{
		...
		if (c.GetTerrain(map).exposesToVacuum || !c.GetAffordances(map).Contains(ThingDefOf.Shuttle.terrainAffordanceNeeded))
		{
			return false;
		}
