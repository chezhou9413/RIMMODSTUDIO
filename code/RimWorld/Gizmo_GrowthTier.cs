...
public class Gizmo_GrowthTier : Gizmo
{
	...
	private string GrowthTierTooltip(int tier)
	{
		...
		{
			taggedString += ("ProgressToNextGrowthTier".Translate() + ": ").AsTipTitle() + Mathf.FloorToInt(child.ageTracker.growthPoints) + " / " + GrowthUtility.GrowthTiers[tier + 1].pointsRequirement;
	...
	private void DrawGrowthTier(Rect rect)
	{
		...
		string label2 = (child.ageTracker.AtMaxGrowthTier ? (pointsRequirement + " / " + pointsRequirement) : (Mathf.FloorToInt(child.ageTracker.growthPoints) + " / " + GrowthUtility.GrowthTiers[growthTier + 1].pointsRequirement));
