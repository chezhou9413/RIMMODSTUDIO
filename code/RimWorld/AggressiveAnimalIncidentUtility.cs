...
public static class AggressiveAnimalIncidentUtility
{
	...
	public static bool TryFindAggressiveAnimalKind(float points, Map map, out PawnKindDef animalKind)
	{
		...
		List<PawnKindDef> animals = (from k in map.Biomes.SelectMany((BiomeDef b) => b.AllWildAnimals)
		...
		animals = DefDatabase<PawnKindDef>.AllDefs.Where((PawnKindDef k) => CanArriveManhunter(k) && CanArriveWithPollution(k, map, polluted) && (!map.Tile.Valid || Find.World.tileTemperatures.SeasonAndOutdoorTemperatureAcceptableFor(map.Tile, k.race))).ToList();
		return TryGetAnimalFromList(points, animals, out animalKind);
	}
	...
	private static bool CanArriveWithPollution(PawnKindDef k, Map map, bool polluted)
	{
		...
		{
			if (!map.Biomes.Any((BiomeDef b) => b.CommonalityOfAnimal(k) > 0f))
			{
				return map.Biomes.Any((BiomeDef b) => b.CommonalityOfCoastalAnimal(k) > 0f);
			}
			return true;
		}
