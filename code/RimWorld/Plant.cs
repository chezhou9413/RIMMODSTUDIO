...
public class Plant : ThingWithComps
{
	...
	public override void Print(SectionLayer layer)
	{
		...
		foreach (int num5 in positionIndices)
		{
			...
			Graphic.TryGetTextureAtlasReplacementInfo(material, def.category.ToAtlasGroup(), flipUv, vertexColors: false, out material, out var uvs, out var _);
			PlantUtility.SetWindExposureColors(workingColors, this);
			Printer_Plane.PrintPlane(size: new Vector2(num3, num3), layer: layer, center: center, mat: material, rot: 0f, flipUv: flipUv, uvs: uvs, colors: workingColors, topVerticesAltitudeBias: 0.1f, uvzPayload: this.HashOffset() % 1024);
			num4++;
			if (num4 >= num)
			{
				break;
