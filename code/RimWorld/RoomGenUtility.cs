...
public static class RoomGenUtility
{
	...
	public static void FillPrefabsAroundEdges(LayoutPrefabParms parms, int count, LayoutRoom room, Map map, Func<IntVec3, Rot4, bool> validator = null, List<Thing> spawned = null, int contractedBy = 1, bool avoidDoors = true, Faction faction = null, Func<PrefabThingData, Tuple<ThingDef, ThingDef>> overrideSpawnData = null)
	{
		...
		(Rot4, CellRect) tuple = default((Rot4, CellRect));
		for (int i = 0; i < count; i++)
		{
			if (num >= count)
			{
			...
			edgeSpawnOptions.RandomElement().Deconstruct(ref intVec, ref tuple);
			(Rot4, CellRect) tuple2 = tuple;
			IntVec3 intVec2 = intVec;
			var (dir, ignore) = tuple2;
			edgeSpawnOptions.Remove(intVec2);
			Rot4 rot = dir.Rotated(parms.rotOffset);
			if (!parms.def.rotations.HasFlag((RotEnum)rot))
			{
				continue;
			}
			CellRect cellRect = intVec2.RectAbout(parms.def.size, rot);
			IntVec3 offset = -dir.FacingCell;
			int num2 = room.Boundary.GetSideLength(dir.Rotated(RotationDirection.Clockwise));
			while (!cellRect.GetEdgeCells(dir).Contains(intVec2) && --num2 > 0)
			{
	...
	{
		CellRect item = rect.ContractedBy(innerWallsContract);
		int a = Mathf.FloorToInt((float)item.Width / (float)minRoomSize);
		int a2 = Mathf.FloorToInt((float)item.Height / (float)minRoomSize);
		...
		for (int i = 0; i < num; i++)
		{
			...
			CellRect item2 = tuple.left;
			CellRect item3 = tuple.right;
		...
		for (int j = 0; j < num2; j++)
		{
			...
			CellRect item4 = tuple2.bottom;
			CellRect item5 = tuple2.up;
		...
		if (num3 >= 0)
		{
			for (int k = item.minZ; k <= item.maxZ; k++)
			{
				ThingDef obj = wall ?? ThingDefOf.Wall;
				GenSpawn.Spawn(ThingMaker.MakeThing(obj, obj.MadeFromStuff ? (wallStuff ?? ThingDefOf.Steel) : null), new IntVec3(num3, 0, k), map);
			}
		}
		if (num4 >= 0)
		{
			for (int l = item.minX; l <= item.maxX; l++)
			{
				ThingDef obj2 = wall ?? ThingDefOf.Wall;
				GenSpawn.Spawn(ThingMaker.MakeThing(obj2, obj2.MadeFromStuff ? (wallStuff ?? ThingDefOf.Steel) : null), new IntVec3(l, 0, num4), map);
			}
		}
		return queue;
	}
	...
	{
		List<IntVec3> list = (from c in room.rects.SelectMany((CellRect r) => r.ContractedBy(1).Corners)
			where c.GetEdifice(map) == null
			select c).ToList();
		if (list.Count != 0)
		{
