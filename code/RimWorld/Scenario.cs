...
public class Scenario : IExposable, WorkshopUploadable
{
	...
	public override int GetHashCode()
	{
		...
		num ^= ((object)System.Runtime.CompilerServices.Unsafe.As<PublishedFileId_t, PublishedFileId_t>(ref publishedFileIdInt)/*cast due to .constrained prefix*/).GetHashCode();
		for (int i = 0; i < parts.Count; i++)
		{
			if (parts[i] != null)
			{
