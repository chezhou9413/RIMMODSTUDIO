...
public static class GravshipPlacementUtility
{
	...
	private static void SpawnPawns(Gravship gravship, Map map, IntVec3 root)
	{
		using (new ProfilerBlock("Pawns"))
		{
			...
			PositionData.Data data = default(PositionData.Data);
			foreach (KeyValuePair<Pawn, PositionData.Data> pawnPlacement in gravship.PawnPlacements)
			{
				pawnPlacement.Deconstruct(ref pawn, ref data);
				Pawn pawn2 = pawn;
				PositionData.Data data2 = data;
	...
	private static void PowerBuildings(Gravship gravship)
	{
		using (new ProfilerBlock("Power"))
		{
			...
			{
				item.Deconstruct(ref thing, ref flag);
				Thing thing2 = thing;
				bool powerOn = flag;
				thing2.TryGetComp<CompPowerTrader>().PowerOn = powerOn;
			}
		}
	}
	...
	private static void SpawnTerrain(Map map, IntVec3 root, Dictionary<IntVec3, TerrainDef> data, bool clear = false)
	{
		...
		{
			datum.Deconstruct(ref intVec, ref terrainDef);
			IntVec3 intVec2 = intVec;
			TerrainDef terrainDef2 = terrainDef;
			IntVec3 intVec3 = root + intVec2;
			...
			if (clear)
			{
				ClearThingsAt(intVec3, map, ClearMode.All);
			}
			if (ModsConfig.BiotechActive)
			{
				map.pollutionGrid.SetPolluted(intVec3, isPolluted: false, silent: true);
			}
			map.snowGrid.SetDepth(intVec3, 0f);
			map.sandGrid.SetDepth(intVec3, 0f);
			if (!terrainDef2.isFoundation && !intVec3.SupportsStructureType(map, terrainDef2.terrainAffordanceNeeded))
			{
				map.terrainGrid.SetTerrain(intVec3, map.Biome.TerrainForAffordance(terrainDef2.terrainAffordanceNeeded));
			}
			if (terrainDef2.isFoundation)
			{
				if (map.terrainGrid.CanRemoveTopLayerAt(intVec3))
				{
					map.terrainGrid.RemoveTopLayer(intVec3, doLeavings: false);
				}
				map.terrainGrid.SetFoundation(intVec3, terrainDef2);
			}
			else
			{
				map.terrainGrid.SetTerrain(intVec3, terrainDef2);
			}
			map.fogGrid.FloodUnfogAdjacent(intVec3);
		}
	}
