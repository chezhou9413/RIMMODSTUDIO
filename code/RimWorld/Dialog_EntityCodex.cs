...
public class Dialog_EntityCodex : Window
{
	...
	public Dialog_EntityCodex(EntityCodexEntryDef selectedEntry = null)
	{
		...
		EntityCategoryDef entityCategoryDef = default(EntityCategoryDef);
		List<EntityCodexEntryDef> list = default(List<EntityCodexEntryDef>);
		foreach (KeyValuePair<EntityCategoryDef, List<EntityCodexEntryDef>> item3 in entriesByCategory)
		{
			item3.Deconstruct(ref entityCategoryDef, ref list);
			list.SortBy((EntityCodexEntryDef e) => e.orderInCategory, (EntityCodexEntryDef e) => e.label);
		}
