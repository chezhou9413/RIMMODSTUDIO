	...
	public override Job NonScanJob(Pawn pawn)
	{
		...
		if (pawn.CanReserve(pawn.Position, 1, -1, ReservationLayerDefOf.Floor) && !pawn.Position.GetTerrain(pawn.Map).IsWater)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				if (intVec.InBounds(pawn.Map) && intVec.GetZone(pawn.Map) is Zone_Fishing { ShouldFishNow: not false } && !intVec.IsForbidden(pawn) && pawn.CanReserve(intVec, 1, -1, ReservationLayerDefOf.Floor))
				{
		...
		{
			if (allZone is Zone_Fishing { ShouldFishNow: not false } zone_Fishing3 && (zone_Fishing2 == null || (float)pawn.Position.DistanceToSquared(zone_Fishing3.Cells[0]) < num))
			{
