...
public class GameComponent_Anomaly : GameComponent
{
	...
	private void UpdateHypnotized()
	{
		...
		int num = default(int);
		foreach (KeyValuePair<Pawn, int> hypnotisedPawn in hypnotisedPawns)
		{
			hypnotisedPawn.Deconstruct(ref pawn, ref num);
			Pawn item = pawn;
			int num2 = num;
	...
	private void UpdateCorpseTrackers()
	{
		...
		Pawn pawn = default(Pawn);
		UnnaturalCorpseTracker unnaturalCorpseTracker = default(UnnaturalCorpseTracker);
		foreach (KeyValuePair<Pawn, UnnaturalCorpseTracker> corpseTracker in corpseTrackers)
		{
			corpseTracker.Deconstruct(ref pawn, ref unnaturalCorpseTracker);
			Pawn item = pawn;
			UnnaturalCorpseTracker unnaturalCorpseTracker2 = unnaturalCorpseTracker;
	...
	public bool TryGetUnnaturalCorpseTrackerForAwoken(Pawn pawn, out UnnaturalCorpseTracker tracker)
	{
		...
		foreach (KeyValuePair<Pawn, UnnaturalCorpseTracker> corpseTracker in corpseTrackers)
		{
			corpseTracker.Deconstruct(ref pawn2, ref unnaturalCorpseTracker);
			UnnaturalCorpseTracker unnaturalCorpseTracker2 = unnaturalCorpseTracker;
	...
	public override void ExposeData()
	{
		...
		if (Scribe.mode == LoadSaveMode.Saving)
		{
			...
			Pawn pawn = default(Pawn);
			int num = default(int);
			foreach (KeyValuePair<Pawn, int> item in hypnotisedPawns.Where((KeyValuePair<Pawn, int> kvp) => kvp.Key.DestroyedOrNull() || kvp.Key.Dead).ToList())
			{
				item.Deconstruct(ref pawn, ref num);
				Pawn key = pawn;
