...
{
	private CellRect structureRect;
	...
	public override void GenerateCriticalStructures(Map map)
	{
		if (ModsConfig.OdysseyActive)
		{
			...
			if (!MapGenUtility.TryGetStructureRect(map, structureSizeRange, out structureRect))
			{
				structureRect = map.Center.RectAbout(structureSizeRange.RandomInRange, structureSizeRange.RandomInRange);
			}
			orGenerateVar.Add(structureRect);
			MapGenerator.SetVar("SpawnRect", structureRect);
			StructureGenParams parms = new StructureGenParams
			{
				size = structureRect.ContractedBy(def.structureGenParms.perimeterExpandBy).Size
			};
			...
			worker.Spawn(layoutStructureSketch, map, structureRect.ContractedBy(def.structureGenParms.perimeterExpandBy).Min);
	...
	{
		MapGenUtility.SpawnScatteredGroupPrefabs(map, structureRect, def.structureGenParms.scatteredPrefabs);
	}
