...
public class Building_OutfitStand : Building, IThingHolderEvents<Thing>, IStorageGroupMember, IHaulEnroute, ILoadReferenceable, IHaulDestination, IStoreSettingsParent, IHaulSource, IThingHolder, IApparelSource, ISearchableContents, IBeautyContainer
{
	...
	{
		base.ExposeData();
		if (Scribe.mode == LoadSaveMode.LoadingVars && ScribeMetaHeaderUtility.loadedGameVersionBuild <= 4466)
		{
			ThingOwner<Apparel> target = new ThingOwner<Apparel>(this);
			Scribe_Deep.Look(ref target, "innerContainer", this);
			innerContainer = new ThingOwner<Thing>(this);
			foreach (Apparel item in target.InnerListForReading.ToList())
			{
				target.TryTransferToContainer(item, innerContainer, canMergeWithExistingStacks: false);
			}
		}
		else
		{
		...
		foreach (Thing item2 in innerContainer.InnerListForReading)
		{
			if (!(item2 is Apparel) && item2.def.IsWeapon)
			{
				holdingWeaponCached = true;
				break;
			}
