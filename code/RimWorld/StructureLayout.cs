...
public class StructureLayout : IExposable
{
	...
	public IEnumerable<(LayoutRoom, CellRect a, CellRect b)> GetLogicalRoomConnections(LayoutRoom room)
	{
		...
		foreach (LayoutRoom otherRoom in Rooms)
		{
			...
			{
				foreach (KeyValuePair<CellRect, List<Vector2>> item2 in connections)
				{
					item2.Deconstruct(ref cellRect, ref list);
					CellRect item = cellRect;
					foreach (Vector2 item3 in list)
					{
						if (rect2.Contains(new IntVec3(Mathf.RoundToInt(item3.x), 0, Mathf.RoundToInt(item3.y))))
						{
							found = true;
							break;
						}
	...
	public void RemoveLogicalConnectionsBetweenRooms(LayoutRoom a, LayoutRoom b)
	{
		...
		foreach (KeyValuePair<Vector2, List<Vector2>> connection in neighbours.connections)
		{
			connection.Deconstruct(ref vector, ref list);
			Vector2 point = vector;
			List<Vector2> connections = list;
