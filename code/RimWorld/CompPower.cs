...
public abstract class CompPower : ThingComp
{
	...
	{
		if (connectChildren.NullOrEmpty())
		{
			return;
		}
		for (int num = connectChildren.Count - 1; num >= 0; num--)
		{
			CompPower compPower = connectChildren[num];
			if (!Find.CurrentGravship.ContainsThing(compPower.parent))
			{
				connectChildren.Remove(compPower);
			}
		}
	}
