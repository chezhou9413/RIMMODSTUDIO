...
public static class GravshipUtility
{
	...
	public static Gravship GenerateGravship(Building_GravEngine engine)
	{
		...
		foreach (Thing item in list)
		{
			...
			if (item.Spawned)
			{
				item.DeSpawn(DestroyMode.WillReplace);
			}
		}
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			foreach (Designation item2 in map.designationManager.AllDesignationsAt(intVec))
			{
				map.designationManager.RemoveDesignation(item2);
			}
	...
	public static bool TryFindSpotOnGravship(Pawn pawn, Building_GravEngine engine, out IntVec3 spot)
	{
		...
		Region region = ClosestShipRegion(pawn.Position, pawn.MapHeld, pawn, TraverseParms.For(pawn));
		if (region == null)
		{
			desperate = true;
			region = ClosestShipRegion(pawn.Position, pawn.MapHeld, pawn, TraverseParms.For(pawn), desperate: true);
		}
		...
		return false;
		Region ClosestShipRegion(IntVec3 root, Map map, Pawn pawn2, TraverseParms traverseParms, bool desperate2 = false)
		{
		...
		bool TryGetAllowedCellInRegion(Region region2, Pawn pawn2, out IntVec3 cell, bool desperate2 = false)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				if (IsOnboardGravship(randomCell, engine, pawn2, desperate2))
				{
					cell = randomCell;
					return true;
				}
			...
			{
				if (IsOnboardGravship(item, engine, pawn2, desperate2))
				{
					cell = item;
					return true;
				}
