...
public static class PortraitsCache
{
	...
	{
		PortraitParams portraitParams = default(PortraitParams);
		Dictionary<Pawn, CachedPortrait> dictionary = default(Dictionary<Pawn, CachedPortrait>);
		foreach (KeyValuePair<PortraitParams, Dictionary<Pawn, CachedPortrait>> cachedPortrait in cachedPortraits)
		{
			cachedPortrait.Deconstruct(ref portraitParams, ref dictionary);
			Dictionary<Pawn, CachedPortrait> dictionary2 = dictionary;
	...
	private static void SetAnimatedPortraitsDirty()
	{
		PortraitParams portraitParams = default(PortraitParams);
		Dictionary<Pawn, CachedPortrait> dictionary = default(Dictionary<Pawn, CachedPortrait>);
		foreach (KeyValuePair<PortraitParams, Dictionary<Pawn, CachedPortrait>> cachedPortrait2 in cachedPortraits)
		{
			cachedPortrait2.Deconstruct(ref portraitParams, ref dictionary);
			Dictionary<Pawn, CachedPortrait> dictionary2 = dictionary;
			toSetDirty.Clear();
			foreach (KeyValuePair<Pawn, CachedPortrait> item in dictionary2)
			{
