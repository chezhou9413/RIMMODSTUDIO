...
public class TileMutatorWorker_RiverDelta : TileMutatorWorker_River
{
	...
	private void SplitRiver(Map map, RiverNode prevSegment, float baseAngle)
	{
		if (prevSegment.end.InBounds(map) && !map.terrainGrid.TerrainAt(prevSegment.end.ToIntVec3()).IsOcean)
		{
			...
			if (!Rand.Chance(0.9f))
			{
				float f = (baseAngle + SplitAngleRange.RandomInRange - SplitAngleRange.Average) * ((float)Math.PI / 180f);
			...
			float f2 = (baseAngle - SplitAngleRange.RandomInRange - num) * ((float)Math.PI / 180f);
			...
			f2 = (baseAngle + SplitAngleRange.RandomInRange + num) * ((float)Math.PI / 180f);
