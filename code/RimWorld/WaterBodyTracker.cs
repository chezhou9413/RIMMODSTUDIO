...
public class WaterBodyTracker : IExposable
{
	...
	private void TryCreateBodyOrAddToExisting(IntVec3 c, WaterBodyType waterBodyType)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (c2.InBounds(map) && c2.GetWaterBodyType(map) == waterBodyType && TryGetWaterBodyAt(c2, out var body2))
			{
				FloodFillBody(c, body2);
				return;
			}
		}
		WaterBody waterBody = new WaterBody(map, c);
		bodies.Add(waterBody);
		waterBody.Initialize();
		bodiesByCell.SetOrAdd(c, waterBody);
		waterBody.cells.Add(c);
		if (waterBody.MaxPopulation > 0f)
		{
			anyBodyContainsFish = true;
		}
	}
