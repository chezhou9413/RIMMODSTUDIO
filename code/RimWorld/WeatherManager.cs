...
public sealed class WeatherManager : IExposable
{
	...
	public void EndAllSustainers()
	{
		...
		Sustainer sustainer = default(Sustainer);
		foreach (KeyValuePair<SoundDef, Sustainer> ambienceSustainer in ambienceSustainers)
		{
			ambienceSustainer.Deconstruct(ref soundDef, ref sustainer);
			sustainer.End();
		}
		ambienceSustainers.Clear();
	}
	...
	private void SetAmbienceSustainersVolume()
	{
		...
		{
			ambienceSustainer.Deconstruct(ref soundDef, ref sustainer);
			SoundDef soundDef2 = soundDef;
			Sustainer sustainer2 = sustainer;
			float num = VolumeOfAmbientSound(soundDef2);
			if (num > 0.0001f)
			{
				sustainer2.externalParams["LerpFactor"] = num;
				continue;
			}
			sustainer2.End();
			TmpToRemove.Add(soundDef2);
		}
