...
public static class IdeoUIUtility
{
	...
	{
		IssueDef issueDef = default(IssueDef);
		List<FloatMenuOption> list = default(List<FloatMenuOption>);
		foreach (KeyValuePair<IssueDef, List<FloatMenuOption>> item in issueGroupsTmp)
		{
			item.Deconstruct(ref issueDef, ref list);
			IssueDef issue = issueDef;
			List<FloatMenuOption> list2 = list;
			if (!list2.Empty())
			{
				options.Add(new FloatMenuOption(issue.LabelCap, delegate
				{
					Find.WindowStack.Add(new FloatMenu(list2, issue.LabelCap));
				}, issue.Icon ?? ideo.Icon, Color.white));
			}
		}
		PreceptDef preceptDef = default(PreceptDef);
		foreach (KeyValuePair<IssueDef, PreceptDef> item2 in singlePreceptsTmp)
		{
			item2.Deconstruct(ref issueDef, ref preceptDef);
			PreceptDef def = preceptDef;
