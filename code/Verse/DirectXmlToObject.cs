	...
	private static List<T> ListFromXml<T>(XmlNode listRootNode)
	{
		...
		try
		{
			...
			foreach (XmlNode childNode in listRootNode.ChildNodes)
			{
				...
				else if (xmlAttribute2 == null || xmlAttribute2.Value.NullOrEmpty() || ModsConfig.IsAnyActiveOrEmpty(xmlAttribute2.Value.Split(',', StringSplitOptions.None), trimNames: true))
				{
					try
					{
