...
public class PathFinder : IDisposable
{
	...
	private struct PathUniqueState : IDisposable
	{
		...
		public void Initialize(int cells)
		{
			...
			excludedDestIndices = new NativeList<int>(4, AllocatorHandle.op_Implicit(Allocator.Persistent));
	...
	private struct PathJobOutput : IDisposable
	{
		...
		public void Initialize(int cells)
		{
			...
			path = new NativeList<IntVec3>(cells, AllocatorHandle.op_Implicit(Allocator.Persistent));
			data = new NativeReference<PathFinderJob.ResultData>(default(PathFinderJob.ResultData), AllocatorHandle.op_Implicit(Allocator.Persistent));
		}
	...
	private ScheduledGridJob ScheduleGridJob(PathRequest request, JobHandle handle = default(JobHandle))
	{
		...
		scheduledGridJob.handle = (handle.Equals(default(JobHandle)) ? job.Schedule(numGridCells, idealBatchCount) : job.Schedule(numGridCells, idealBatchCount, handle));
	...
	{
		if (!gridJobOutputs.TryDequeue(ref output))
		{
	...
	{
		if (!pathUniqueStates.TryDequeue(ref state))
		{
	...
	{
		if (!pathJobOutputs.TryDequeue(ref output))
		{
	...
	private static void MakeDestination(LocalTargetInfo target, PathEndMode peMode, PathingContext context, out CellRect rect, ref NativeList<int> excluded)
	{
		...
		if (peMode == PathEndMode.Touch && rect.Area != 1)
		{
			...
			bool forEdifice = target.Thing?.def.IsEdifice() ?? false;
			if (BL.InBounds(map) && !TouchPathEndModeUtility.IsCornerTouchAllowed(BL + intVec + intVec2, BL + intVec2, BL + intVec, context, forEdifice))
			{
				int num = cellIndices.CellToIndex(BL);
				excluded.Add(ref num);
			}
			if (TR.InBounds(map) && !TouchPathEndModeUtility.IsCornerTouchAllowed(TR - intVec - intVec2, TR - intVec2, TR - intVec, context, forEdifice))
			{
				int num = cellIndices.CellToIndex(TR);
				excluded.Add(ref num);
			}
			if (TL.InBounds(map) && !TouchPathEndModeUtility.IsCornerTouchAllowed(TL + intVec - intVec2, TL - intVec2, TL + intVec, context, forEdifice))
			{
				int num = cellIndices.CellToIndex(TL);
				excluded.Add(ref num);
			}
			if (BR.InBounds(map) && !TouchPathEndModeUtility.IsCornerTouchAllowed(BR - intVec + intVec2, BR + intVec2, BR - intVec, context, forEdifice))
			{
	...
	public void LogPathfinderState()
	{
		...
		MapGridRequest mapGridRequest = default(MapGridRequest);
		ScheduledGridJob scheduledGridJob = default(ScheduledGridJob);
		foreach (KeyValuePair<MapGridRequest, ScheduledGridJob> item in gridJobLookup)
		{
			item.Deconstruct(ref mapGridRequest, ref scheduledGridJob);
			MapGridRequest mapGridRequest2 = mapGridRequest;
			ScheduledGridJob scheduledGridJob2 = scheduledGridJob;
	...
	public void LogGridCellResult(IntVec3 cell)
	{
		...
		foreach (KeyValuePair<MapGridRequest, ScheduledGridJob> item in gridJobLookup)
		{
			...
			MapGridRequest mapGridRequest2 = mapGridRequest;
			ScheduledGridJob scheduledGridJob2 = scheduledGridJob;
			ScheduledGridJob local = scheduledGridJob2;
			list.Add(new FloatMenuOption($"{mapGridRequest2} ({(GenTicks.TicksGame - scheduledGridJob2.computed).TicksToSeconds():F2}s old)", delegate
			{
