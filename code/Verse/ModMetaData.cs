...
public class ModMetaData : WorkshopUploadable
{
	...
	public IEnumerable<string> Authors
	{
		get
		{
			...
			else
			{
				...
				foreach (string item in meta.author.Split(AndToken, StringSplitOptions.RemoveEmptyEntries).SelectMany((string x) => x.Split(',', StringSplitOptions.None)))
				{
					yield return item;
				}
			}
	...
	private void Init()
	{
		...
		meta = DirectXmlLoader.ItemFromXmlFile<ModMetaDataInternal>(GenFile.ResolveCaseInsensitiveFilePath(RootDir.FullName + Path.DirectorySeparatorChar + "About", "About.xml"), resolveCrossRefs: false);
	...
	public unsafe void SetPublishedFileId(PublishedFileId_t newPfid)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		...
		if (!(publishedFileIdInt == newPfid))
		{
			...
			File.WriteAllText(PublishedFileIdPath, ((object)(*(PublishedFileId_t*)(&newPfid))/*cast due to .constrained prefix*/).ToString());
		}
	}
