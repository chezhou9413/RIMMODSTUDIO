...
public static class DebugActionsMisc
{
	...
	private static void LayerPathfinder()
	{
		...
		foreach (KeyValuePair<int, PlanetLayer> planetLayer5 in Find.WorldGrid.PlanetLayers)
		{
			planetLayer5.Deconstruct(ref num, ref planetLayer);
			PlanetLayer planetLayer2 = planetLayer;
			PlanetLayer origin = planetLayer2;
			list.Add(new DebugMenuOption(origin.Def.label, DebugMenuOptionMode.Action, delegate
			{
				List<DebugMenuOption> list2 = new List<DebugMenuOption>();
				int num2 = default(int);
				PlanetLayer planetLayer3 = default(PlanetLayer);
				foreach (KeyValuePair<int, PlanetLayer> planetLayer6 in Find.WorldGrid.PlanetLayers)
				{
					planetLayer6.Deconstruct(ref num2, ref planetLayer3);
					PlanetLayer planetLayer4 = planetLayer3;
					PlanetLayer dest = planetLayer4;
					if (dest != origin)
					{
