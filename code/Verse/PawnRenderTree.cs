...
public class PawnRenderTree
{
	...
	private void TrySetupGraphIfNeeded()
	{
		...
		using (new ProfilerBlock("TrySetupGraph"))
		{
			...
			List<PawnRenderNode> list = default(List<PawnRenderNode>);
			foreach (KeyValuePair<PawnRenderNodeTagDef, List<PawnRenderNode>> tmpChildTagNode in tmpChildTagNodes)
			{
				tmpChildTagNode.Deconstruct(ref pawnRenderNodeTagDef, ref list);
				PawnRenderNodeTagDef key = pawnRenderNodeTagDef;
				List<PawnRenderNode> list2 = list;
