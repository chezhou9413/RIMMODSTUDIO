...
public sealed class TerrainGrid : IExposable
{
	...
	private void DoTerrainChangedEffects(IntVec3 c, TerrainDef oldTerr, TerrainDef newTerr, TerrainDef oldFoundation = null)
	{
		...
		for (int num = thingList.Count - 1; num >= 0; num--)
		{
			...
			{
				if ((!newTerr.IsFlood || thingList[num].def.plant.destroyedByFlooding) && !thingList[num].def.plant.completelyIgnoreFertility && map.fertilityGrid.FertilityAt(c) < thingList[num].def.plant.fertilityMin)
				{
