...
public struct PathFinderCostTuning : IEquatable<PathFinderCostTuning>
{
	...
	{
		return HashCode.Combine<int, int>(HashCode.Combine<int, float, int, int, float, int, int, int>(costBlockedWallBase, costBlockedWallExtraPerHitPoint, costBlockedWallExtraForNaturalWalls, costBlockedDoor, costBlockedDoorPerHitPoint, costOffLordWalkGrid, costDanger, costBlueprintFrame), costWater);
	}
}
