...
public sealed class DynamicDrawManager
{
	...
	private struct CullJob : IJobParallelFor
	{
		...
		public void Execute(int index)
		{
			...
			if (!indices.Contains(num) || (!value.seeThroughFog && ((NativeBitArray)(ref fogGrid)).IsSet(num)) || (value.hideAtSnowOrSandDepth < 1f && depthGrid[num] > value.hideAtSnowOrSandDepth))
			{
				return;
			}
