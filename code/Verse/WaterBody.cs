...
public class WaterBody : IExposable
{
	...
	public void SetFishTypes()
	{
		...
		commonFish.Clear();
		uncommonFish.Clear();
		if (!Rand.Chance(FishingUtility.ChanceForCommonFishFromWaterBodySizeCurve.Evaluate(Size)))
		{
			return;
		}
	...
	public void ExposeData()
	{
		...
		{
			if (commonFish == null || uncommonFish == null)
			{
				commonFish = new List<ThingDef>();
				uncommonFish = new List<ThingDef>();
				SetFishTypes();
			}
			if (Population > 0f && commonFish.Count == 0)
			{
				SetFishTypes();
			}
