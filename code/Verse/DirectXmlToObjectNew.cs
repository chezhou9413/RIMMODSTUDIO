...
public static class DirectXmlToObjectNew
{
	...
	private static void EmitIlToCreateCustomLoadable(ILGenerator il, Type typeToInstantiate)
	{
		...
		MethodInfo method = typeToInstantiate.GetMethod("LoadDataFromXmlCustom", BindingFlags.Instance | BindingFlags.Public);
		if (constructor == null)
		{
	...
	public static bool ValidateMayRequires(string mayRequire, string mayRequireAny)
	{
		...
		if (!mayRequireAny.NullOrEmpty() && !ModsConfig.IsAnyActiveOrEmpty(mayRequireAny.Split(',', StringSplitOptions.None), trimNames: true))
		{
			return false;
		}
		return true;
