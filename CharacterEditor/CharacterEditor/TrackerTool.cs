...
internal static class TrackerTool
{
	...
	internal static bool HasMutantTracker(this Pawn p)
	{
		return p != null && p.mutant != null;
	}
	...
	internal static void FixInvalidTracker(this Pawn newPawn)
	{
		try
		{
			if (newPawn == null || newPawn.Faction.IsCreature() || newPawn.kindDef.IsMechanoid() || newPawn.kindDef.IsZombie() || newPawn.kindDef.IsAbomination() || newPawn.kindDef.RaceProps.Animal)
			{
				return;
			}
			bool flag = false;
			...
			if (newPawn.outfits != null && newPawn.outfits.CurrentApparelPolicy != null && newPawn.outfits.CurrentApparelPolicy.filter != null)
			{
