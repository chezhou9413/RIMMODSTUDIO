...
internal class PresetPawn
{
	...
	private string AsString
	{
		...
		{
			string text = "";
			foreach (Param key in dicParams.Keys)
			{
	...
	private string Save(int slot)
	{
		...
		CEditor.API.SetSlot(slot, asString, andSave: true);
		return asString;
	}

	public bool Load(int slot, string custom)
	{
		...
		if (!string.IsNullOrEmpty(custom))
		{
			...
			if (array.Length >= MinParams)
			{
				...
				foreach (string value in array2)
				{
					...
					{
						dicParams.Add((Param)dicParams.Count, value);
					}
				}
			}
			if (array.Length < ParamCount)
			{
				int num = ParamCount - array.Length;
				array = array.AddSeqRange(new string[num]);
				Log.Message(num + " parameter in pawn slotdata " + array[0] + " were missing - using dummyvalues");
			}
		}
	...
	internal string SavePawn(Pawn p, int slot)
	{
		...
		dicParams.Add(Param.P09_crowntype, "0".ToString());
		...
		dicParams.Add(Param.P14_haircolor1, p.GetHairColor(primary: true).ColorHexString());
		dicParams.Add(Param.P15_haircolor2, p.GetHairColor(primary: false).ColorHexString());
		dicParams.Add(Param.P16_melanin, "-1".ToString());
		dicParams.Add(Param.P17_skincolor1, p.GetSkinColor(primary: true).ColorHexString());
		dicParams.Add(Param.P18_skincolor2, p.GetSkinColor(primary: false).ColorHexString());
		...
		dicParams.Add(Param.P23_backstorychild, p.GetBackstrory(isChildhood: true));
		dicParams.Add(Param.P24_backstoryadult, p.GetBackstrory(isChildhood: false));
		...
		dicParams.Add(Param.P41_favcolor, p.GetFavColorDepricated().ColorHexString());
		...
		dicParams.Add(Param.P53_eyecolor2, p.GetEyeColor2().ColorHexString());
		dicParams.Add(Param.P54_mutantDef, p.GetMutantDefName());
		dicParams.Add(Param.P55_rotStage, p.GetMutantRotStage());
		dicParams.Add(Param.P56_favColorDef, p.GetFavColorDefName());
		if (Prefs.DevMode)
		{
	...
	private void Clearing(Pawn pawn)
	{
		try
		{
	...
	private void TryAddPsy(Pawn pawn, int paramCount, string entropy, string psyfocus, string psyabilities)
	{
		try
		{
			if (ModsConfig.RoyaltyActive)
			{
				if (paramCount > 38)
				{
					pawn.psychicEntropy.TryAddEntropy(entropy.AsFloat());
				}
				if (paramCount > 39)
				{
					pawn.SetPsyfocus(psyfocus.AsFloat());
				}
			}
			if (paramCount > 40)
			{
				pawn.SetPsyAbilitiesFromSeparatedString(psyabilities);
			}
		}
		catch (Exception e)
		{
	...
	internal void GeneratePawn(bool _setBodyParts = true, bool setApparel = true)
	{
		...
		PawnKindDef pawnKindDef = DefTool.PawnKindDef(dicParams.GetValue(Param.P01_pawnkinddef), PawnKindDefOf.Colonist);
		ThingDef raceDef = pawnKindDef?.race;
		Faction bySeparatedString = FactionTool.GetBySeparatedString(dicParams.GetValue(Param.P02_faction), Faction.OfPlayer);
		pawn = PawnxTool.CreateNewPawn(pawnKindDef, bySeparatedString, raceDef);
		if (pawn != null)
		{
			Clearing(pawn);
			...
			if (setBodyParts)
			{
				pawn.SetBodyParts(dicParams.GetValue(Param.P07_bodydefname), dicParams.GetValue(Param.P47_headtypedef), dicParams.GetValue(Param.P11_headaddons), dicParams.GetValue(Param.P12_hairdefname), pawnKindDef, dicParams.Count);
			}
			...
			pawn.SetHairColor(primary: true, dicParams.GetValue(Param.P14_haircolor1).HexStringToColor());
			pawn.SetHairColor(primary: false, dicParams.GetValue(Param.P15_haircolor2).HexStringToColor());
			pawn.SetSkinColor(primary: true, dicParams.GetValue(Param.P17_skincolor1).HexStringToColor(), ensureVisible: true);
			pawn.SetSkinColor(primary: false, dicParams.GetValue(Param.P18_skincolor2).HexStringToColor(), ensureVisible: true);
			pawn.SetEyeColor(dicParams.GetValue(Param.P19_eyecolor).HexStringToColor());
			if (dicParams.Count > 53)
			{
				pawn.SetEyeColor2(dicParams.GetValue(Param.P53_eyecolor2).HexStringToColor());
			}
			...
			{
				TryAddPsy(pawn, dicParams.Count, dicParams.GetValue(Param.P38_entropy), dicParams.GetValue(Param.P39_psyfocus), dicParams.GetValue(Param.P40_psyabilities));
			}
			if (dicParams.Count > 41)
			{
				pawn.SetFavColorDepricated(dicParams.GetValue(Param.P41_favcolor).HexStringToColor());
			}
			...
			{
				MissingMods(dicParams.GetValue(Param.P44_modids), asDialog: false);
			}
			...
			if (dicParams.Count > 55)
			{
				pawn.SetUnsetMutantByDefName(dicParams.GetValue(Param.P54_mutantDef), dicParams.GetValue(Param.P55_rotStage));
			}
			if (dicParams.Count > 56)
			{
				pawn.SetFavColorByDefName(dicParams.GetValue(Param.P56_favColorDef));
			}
			HealthCheck(pawn);
		}
	}

	private void MissingMods(string usedmods, bool asDialog)
	{
		try
		{
			string namesOfInactiveMods = PawnxTool.GetNamesOfInactiveMods(usedmods);
	...
	internal string ShowPawnMods()
	{
		try
		{
			string value = dicParams.GetValue(Param.P44_modids);
			string namesOfInactiveMods = PawnxTool.GetNamesOfInactiveMods(value);
			return "Pawn Mods:\n" + value + "\nMissing Mods:\n" + namesOfInactiveMods;
		}
		catch
		{
	...
	private void HealthCheck(Pawn pawn)
	{
		try
		{
	...
	{
		int num = CEditor.API.NumSlots - 1;
		for (int num2 = num; num2 >= 0; num2--)
		{
			CEditor.API.SetSlot(num2, "", num2 == 0);
		}
	}
