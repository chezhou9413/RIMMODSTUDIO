...
internal class ScenPart_StartingAnimalExtra : ScenPart
{
	...
	public override void ExposeData()
	{
		...
		Scribe_Deep.Look(ref pawnName, "pawnName", null);
	...
	{
		if (other is ScenPart_StartingAnimalExtra scenPart_StartingAnimalExtra && scenPart_StartingAnimalExtra.animalKind == animalKind)
		{
			count += scenPart_StartingAnimalExtra.count;
			return true;
		}
		return false;
	}
	...
	public override IEnumerable<Thing> PlayerStartingThings()
	{
		for (int i = 0; i < count; i++)
		{
			if (animalKind.IsAnimal() || animalKind == null)
			{
				...
				animal.SetAge(age);
				if (gender != Gender.None && animal.gender != gender)
				{
			...
			try
			{
				pawn2 = PawnxTool.CreateNewPawn(animalKind, Faction.OfPlayer, animalKind.race);
				pawn2.SetAge(age);
				if (gender != Gender.None && pawn2.gender != gender)
				{
					pawn2.gender = gender;
				}
				if (pawnName != null)
